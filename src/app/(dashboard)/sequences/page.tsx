      ...
      <div className="rounded-lg border border-neutral-200 bg-white p-6">
        ...
        {accounts.length === 0 ? (
          <p className="text-sm text-neutral-600">
            Connect a Gmail account before creating a sequence.{" "}
            <a href="/settings/accounts" className="font-medium text-neutral-900 hover:underline">
              Connect one now →
            </a>
          </p>
        ) : (
          <form action={createSequence} className="flex flex-wrap gap-2">
            ...
            <select name="sendAsAccountId" required className="rounded-md border border-neutral-300 px-3 py-2 text-sm">
              {accounts.map((a) => (
          ...
          <tbody>
            {sequences.map((s) => (
              <tr key={s.id} className="border-b border-neutral-100 last:border-0 hover:bg-neutral-50">
                ...
                <td className="px-4 py-2 text-neutral-600">{s.sendAsAccount.email}</td>
